...
str_join(frankenstein, collapse = ' ') #collapse means what it inserts in between the vectors 
...
# Split nonsense on line breaks. Remove excess white space from each line.

split_nonsense <- str_split(nonsense, "\n")[[1]] #splits on /n 
split_nonsense #view 
trimmed_nonsense <- str_trim(split_nonsense) 
trimmed_nonsense

sapply(split_nonsense, function(n) str_trim(n, side = c('both')) ) #returns a matrix 

test <- lapply(split_nonsense, str_trim)
test <- unlist(test)
test

# Reassemble the text as a single string.
# Store the result as Nonsense.

nonsense <- paste(test, collapse = " ")
nonsense
...
str_count(frankenstein, " me ") #this is rudimentary though 
str_count(frankenstein, "\\bme\\b") # \\b escapes b, \b and is the word boundary parameter 

# Exercise 4 ----------------------------------------------------------------------------------------------------------

# Find the position of the word "courage" in Frankenstein.

str_locate(frankenstein, "\\bcourage\\b") #retrieves as a matrix, to make a dataframe use 'as.dataframe()'
...
frankenstein <- str_wrap(frankenstein, 40)
frankenstein

cat(frankenstein) #cat takes a string and echoes it to terminal
